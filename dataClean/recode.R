...
library(lavaan)
library(broom)
setwd("/Users/Chris/Dropbox/masterData/Western_States/data/")
...
save(df, file = "/Users/Chris/Dropbox/masterData/Western_States/git/westernStates/data/westernCleaned.rda")


/Users/Chris/Dropbox/masterData/Western_States/git/westernStates/data
# df <- df %>%
#   filter(inputstate == "Arizona")

# library(survey)
# survey_design <- svydesign(ids = ~1, weights = ~psweight, data = df)


# svymean(~female, survey_design)
# svymean(~ ifelse(age < 36, 1, 0), survey_design)
# svymean(~ ifelse(age < 36, 1, 0), survey_design)

# prop.table(svytable(~college, design = survey_design))

# svymean(~treat, survey_design)
# # Why is this giving NaN
...
#   df %>%
#   subset(select = c(
#     "treat", "authoritarianism", "rr", "age", "party3", "rwm", "pid7",
...
#     "trump_vote", "surveillance", "anxiety", "anger", "hope", "pride", "disgust", "enthusiasm", "disposition", "strength"
#   ))
...
#             soft =~ recount + criticize + court " ## It doesn't seem to fit well -- I don't know why
